...
static wchar_t thousands_sep;
static char *grouping; /*FIXME*/
...
#define	PUTC(ch)	(void) putwc(ch, fp)
...
#define CONVERT(type, value, base, string, case)               \
  do {                                                         \
    const wchar_t *digit = (case) ? UPPER_DIGITS : LOWER_DIGITS; \
...
static __inline__ int todigit(wchar_t c)
{
  if (c < L'0') return 0;
  if (c > L'9') return 9;
  return c - L'0';
}
static __inline__ wchar_t tochar(int n)
{
  if (n > 9) return L'9';
  if (n < 0) return L'0';
  return n + L'0';
...
static int doprnt_cvtl(long double number, int prec, int flags, wchar_t *signp,
                       wchar_t fmtch, wchar_t *startp, wchar_t *endp);
static wchar_t *doprnt_roundl(long double fract, int *expv, wchar_t *start,
			   wchar_t *end, wchar_t ch, wchar_t *signp);
static wchar_t *doprnt_exponentl(wchar_t *p, int expv, wchar_t fmtch, int flags);
#ifdef __GO32__
static int doprnt_isspeciall(long double d, wchar_t *bufp, int flags);
#endif
static __inline__ wchar_t * __grouping_format(wchar_t *string_start, wchar_t *string_end, wchar_t *buffer_end, int flags);
static __inline__ va_list __traverse_argument_list(int index_of_arg_to_be_fetched, const wchar_t *format_string, va_list arg_list);

static char NULL_REP[] = "(null)";
static wchar_t WNULL_REP[] = L"(null)";
static const wchar_t LOWER_DIGITS[] = L"0123456789abcdef";
static const wchar_t UPPER_DIGITS[] = L"0123456789ABCDEF";


int
_dowprnt(const wchar_t *fmt0, va_list argp, FILE *fp)
{
  return __dowprnt_chk(fmt0, argp, fp, 0);
}

int
__dowprnt_chk(const wchar_t *fmt0, va_list argp, FILE *fp, int flag)
{
  const wchar_t *fmt;                /* format string */
  int ch;                            /* character from fmt */
  int cnt;                           /* return value accumulator */
  int n;                             /* random handy integer */
  wchar_t *t;                        /* buffer pointer */
  ...
  wchar_t buf[BUF];                  /* space for %c, %[diouxX], %[aAeEfFgG] */
  ...
  const wchar_t *pos;                /* position in format string when checking for numeric conv spec */
  char *nt;                          /* buffer pointer for narrow string conversion */
  char nch;                          /* narrow character for %c */

  fp->_flag |= _IOWIDE;

  locale_info = localeconv();
  decimal_point = btowc(locale_info->decimal_point[0]);
...
rflag:
    ...
    case L'\'':
       ...
       */
      flags |= GROUPING;
      thousands_sep = btowc(locale_info->thousands_sep[0]);
    ...
    case L'c':
      if (flags & LONGINT)
      {
        *(t = buf) = va_arg(argp, wint_t);
      }
      else
      {
        nch = (char)va_arg(argp, int);
        nt = &nch;
        t = NULL;
      }
      size = 1;
	...
	if (ieee_value.ldt.sign)
	{
      ...
      *buf = L'\0';
       ...
       */
      if (softsign || (sign == L'+' && neg_ldouble))
	sign = L'-';
      t = *buf ? buf : buf + 1;
	...
	*va_arg(argp, signed char *) = (signed char)cnt;
      else
	*va_arg(argp, int *) = cnt;
      break;
    case L'O':
...
number:
	...
	if (flags & LONGDBL)
	  ...
	  CONVERT(unsigned long, _ulonglong, base, t, flags & UPPERCASE);
        if ((flags & ALT) && base == 8 && *t != L'0')
          *--t = L'0';  /* octal leading 0 */
      }
...
pforw:
       ...
       * pad with blanks.
       * If `nt' is not NULL, it points to a narrow string to
	...
	PUTC(sign);
      ...
      {
	PUTC(L'0');
	PUTC((*fmt == L'A') ? L'X' : (*fmt == L'a') ? L'x' : *fmt);
      }
	...
	for (n = realsz; n < width; n++)
      ...
      break;
    case L'\0':  /* "%?" prints ?, unless ? is NULL */
      return cnt;
    default:
      PUTC(*fmt);
      cnt++;
...
static int
doprnt_cvtl(long double number, int prec, int flags, wchar_t *signp, wchar_t fmtch,
            wchar_t *startp, wchar_t *endp)
{
  wchar_t *p, *t;
  long double fract = 0;
...
#undef MUST_ROUND_TO_EVEN
    ...
    CONVERT(unsigned long long int, mantissa, 16, p, fmtch == L'A');
    *t++ = *p++;
    *t++ = decimal_point;
    ...
    if (precision_given == false)
    {
      ...
      while (t - fraction_part < prec && t < endp)
        *t++ = L'0';  /*  Pad with zeros to the right.  At the end of the loop points to first free place.  */
    ...
    *--p = (positive_exponent) ? L'+' : L'-';
    *--p = (flags & UPPERCASE) ? L'P' : L'p';
    for (; p < endp; *t++ = *p++)
      ;
...
eformat:
	...
	*t++ = decimal_point;
	...
	do {
	  ...
	  *t++ = tochar((int)tmp);
      ...
      if (fract)
	startp = doprnt_roundl(fract, &expcnt, startp, t - 1, (wchar_t)0, signp);
    }
  ...
  case L'G':
    ...
    if (fract)
      startp = doprnt_roundl(fract, (int *)NULL, startp, t - 1, (wchar_t)0, signp);
...
static wchar_t *
doprnt_roundl(long double fract, int *expv, wchar_t *start, wchar_t *end, wchar_t ch,
	      wchar_t *signp)
{
  long double tmp;
...
static wchar_t *
doprnt_exponentl(wchar_t *p, int expv, wchar_t fmtch, int flags)
{
  wchar_t *t;
  wchar_t expbuf[MAXEXPLD];
...
static int
doprnt_isspeciall(long double d, wchar_t *bufp, int flags)
{
  /*
  ...
  static const wchar_t INF_REP[2][4] = {L"inf", L"INF"};
  static const wchar_t NAN_REP[2][4] = {L"nan", L"NAN"};
  ...
  if (IS_PSEUDO_NUMBER(ieee_value))  /*  Pseudo-NaN, Pseudo-Infinity, Unnormal and Pseudo-Denormal.  */
    ...
    wcscpy(bufp, INF_REP[style]);
  return 3;
}

static __inline__ wchar_t *
__grouping_format(wchar_t *string_start, wchar_t *string_end, wchar_t *buffer_end, int flags)
{
  /*
...
__traverse_argument_list(int index_of_arg_to_be_fetched, const wchar_t *fmt0, va_list argp)
{
  /*
...
rflag:
    ...
    case L'*':
      for (prec_index = 0, fmt++; iswdigit(*fmt); fmt++)
    ...
    case L'5': case L'6': case L'7': case L'8': case L'9':
      for (arg_index = 0; iswdigit(*fmt); fmt++)
        arg_index = 10 * arg_index + todigit(*fmt);
      goto rflag;

    /* Length modifiers */
    case L'h': case L't': case L'z':  /* promoted to int */
      goto rflag;
    case L'L': /* a, A, e, E, f, F, g, G applies to long double */
      flags |= LONGDBL;
      goto rflag;
    case L'l':
      if (flags & LONGINT)
      ...
      goto rflag;
    case L'j': /* d, i o, u, x, X applies to intmax_t */
      flags |= LONGDBL; /* long long */
      goto rflag;
